...
> 终端就是一个可以和电脑“打字说话”的窗口。
> 平时我们用鼠标点按钮来告诉电脑做事。在终端里，我们是把命令打进去，让电脑照着命令做事。比如输入 `git status`，就是在问 Git：“现在有哪些文件变了？”

有时你还会看到两个词：

- **CLI**：全名是 Command Line Interface，意思是“命令行界面”。它主要靠你一行一行输入命令来操作，比如 `git status`。
- **TUI**：全名是 Text User Interface，意思是“文字界面”。它也在终端里打开，但会显示菜单、列表或按钮，通常可以用键盘上下左右选择。
...
| macOS | **终端**（Terminal） |
| Linux | **Terminal**，比如 **GNOME Terminal** 或 **Konsole** |

如果你不知道打开哪一个，可以请老师或家长帮你打开。打开后，看到一个可以输入文字命令的窗口，就对了。
